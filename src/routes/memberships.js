...
// GET /api/memberships — list all members of current institute
...
// POST /api/memberships — create user + add to institute in one step
router.post('/', ...auth, requirePermission('membership.create'), validate(['name', 'email', 'password', 'roleIds']), async (req, res) => {
  try {
    const { name, email, password, roleIds, phone } = req.body;

    // Validate roles exist and belong to this institute or are system roles
    const roles = await Role.find({ _id: { $in: roleIds }, $or: [{ institute: null }, { institute: req.instituteId }], deletedAt: null });
    if (roles.length !== roleIds.length) return res.status(400).json({ success: false, message: 'One or more invalid roles' });

    // Upsert user
    let user = await User.findOne({ email: email.toLowerCase().trim() });
    if (!user) {
      user = await User.create({ name, email: email.toLowerCase().trim(), passwordHash: bcrypt.hashSync(password, 12), phone: phone || null });
    }

    // Check existing membership
    const existing = await InstituteMembership.findOne({ user: user._id, institute: req.instituteId });
    if (existing && !existing.deletedAt) return res.status(409).json({ success: false, message: 'User is already a member of this institute' });
    ...
    logAudit({ userId: req.user._id, instituteId: req.instituteId, action: 'CREATE', resource: 'memberships', resourceId: membership._id, newData: { name, email, roles: roleIds }, req });
...
// PUT /api/memberships/:id — update roles / extra permissions
router.put('/:id', ...auth, requirePermission('membership.update'), async (req, res) => {
  try {
    const { roleIds, extraPermissions, deniedPermissions, isActive } = req.body;
    const membership = await InstituteMembership.findOne({ _id: req.params.id, institute: req.instituteId });
    if (!membership) return res.status(404).json({ success: false, message: 'Membership not found' });

    if (roleIds !== undefined) {
      const roles = await Role.find({ _id: { $in: roleIds }, $or: [{ institute: null }, { institute: req.instituteId }], deletedAt: null });
      if (roles.length !== roleIds.length) return res.status(400).json({ success: false, message: 'Invalid roles' });
    ...
    return res.json({ success: true, data: await membership.populate([{ path: 'user', select: 'name email' }, { path: 'roles', select: 'name displayName' }]) });
  } catch (e) { return res.status(500).json({ success: false, message: e.message }); }
});

// DELETE /api/memberships/:id — remove user from institute (soft)
...
// GET /api/memberships/my — current user's own membership in this institute
